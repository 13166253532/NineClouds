...
module.exports={
    selectDoneDetail : function (conn,ordertype,docentry,callback) {
        var areas = []
        var content = []
        async.series([function (cb) {
            ...
            function (cb) {
                ...
                async.whilst(function () {
                    ...
                    var con = content[i]
                    var _sql = "SELECT t.DATATABLE,t.COLUMNS,t.COLUMNNAMECAN from MDB_FIELD t WHERE t.COLUMNPID ="+sql.to(con.id) + "and columntype = 'column' order by sort asc";
                    sql.executeSql(conn, _sql, function (result) {
                        if (result && result.length>0) {
                            ...
                            async.whilst(function () {
                                ...
                                var rj = result[k]
                                var _sql = "select "+rj.columns+" as a from "+rj.datatable + " where DocEntry = " + sql.to(docentry)
